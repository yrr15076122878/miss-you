...
(function($,win,undef){
	...
	var tipmsg_en={//英文默认提示文字;
		tit:"Prompt information",
		w:{
			"*":"Can not be empty!",
			"*6-16":"Please fill in any characters from 6 to 16!",
			...
			"e":"The mailbox address is not in the right format!",
			"url":"Please fill in the url!"
		},
		def:"Please fill in the correct information!",
		undef:"datatype undefined!",
		reck:"The contents of the two input are inconsistent!",
		r:"",//Through information authentication!
		c:"Testing information...",
		s:"Please{ fill in | choice }{0| information }!",
		v:"The information submitted has not been verified. Please wait a moment...",
		p:"Submitting data....."
	}

	$.Tipmsg=tipmsg=tipmsg_cn;//默认是中文提示
	
	var Validform=function(forms,settings,inited){
		var language = settings.language||'';

		if(language==="en"){
			$.Tipmsg=tipmsg=tipmsg_en;
		}
		...
		forms.each(function(){
			...
			//bind the blur event;
			$this.delegate("[datatype]","blur keyup",function(){
	...
	Validform.util={
		...
		enhance:function(tiptype,usePlugin,tipSweep,addRule){
			...
			//select multiple;
			curform.find("select[datatype][multiple]").bind("click change",function(){
				var _this=$(this);
				setTimeout(function(){
					_this.trigger("blur");
				},0);
		...
		showmsg:function(msg,type,o,triggered){
			...
			*/
			...
			//当自定了提示标签时，就去自定义的。（name=应提示标签的name 附上class=Validform_checktip）
			if((type==3 || type==4) && o.obj){
				var name = o.obj.attr("name");
				var span = $("span[name='"+name+"']");
				var span_sib = o.obj.siblings(".Validform_checktip");
				if(name && span.length>0 && span.hasClass("Validform_checktip")){
					span.html(msg);
					span_sib = span;
				} else{
					o.obj.siblings(".Validform_checktip").html(msg);
				}
				Validform.util.cssctl(span_sib,o.type);
			}
