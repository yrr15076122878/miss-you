...
var main = {
    ...
    alertAddCard: function (dtList) {
        ...
        main.fontAutoSize('.badge-box', '.card_name span:eq(1)', fontSize.nameMinFontSize, arr[2]);
        //main.fontAutoSize('.badge-box', '.card_position span', fontSize.companyMinFontSize, arr[1]);
        main.fontAutoSize('.badge-box', '.card_company span:eq(1)', fontSize.companyMinFontSize, arr[4]);
        //main.fontAutoSize('.badge-box', '.card_category span:eq(1)', fontSize.nameMinFontSize, arr[5]);
    },
    ...
    fontAutoSize: function (parentElem, selfElem, minFont, text) {
		...
		var weight = $(selfElem).parent("div").css('font-weight')
        $(selfElem).css({"font-size": pFont + "px","font-weight":weight});
        var nowFontSize = pFont;
        var selfWidth = 0;
        for (var i = pFont; i > minFont; i--) {
            selfWidth = $(selfElem).width();
            nowFontSize = parseFloat($(selfElem).css('font-size'));
            if (selfWidth > parentElemWidth - 50) {
