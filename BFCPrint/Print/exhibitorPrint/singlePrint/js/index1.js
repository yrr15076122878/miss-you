...
var visitMain = {
    ...
    GetQueryString: function (category,PType) {  //获取Url传入的参数
        ...
        if (r != null) {
            ...
            $('.card_category div').html(scategory);

        } else {
            $("#booth_number,.card_boothNum").hide();
            $('.card_category div').html('');
        }
    },
    ...
    other:function(){
        ...
        if(config.exhibitor){
            $(".card_category").html('展商 EXHIBITOR');
    ...
    searchData:function () {
        $('#btnSearch').on('click ', function () {  //点击搜索按钮执行事件
            $(".getCompany").val("");
            $(".card_company div").html("");
            if($(".getBoothNumb").val()!=""){
                visitMain.AjaxDat("../../data/ERegList.ashx?SeaKey=" + escape($('.getBoothNumb').val()) + "");  //Ajax请求数据
            }else{
        ...
        $(".getBoothNumb").keyup(function (ec) {
            if (ec.keyCode == 13) {
                $(".getCompany").val("");
                $(".card_company div").html("");
                if ($(".getBoothNumb").val() != "") {
    ...
    AjaxDat:function(ajaxUrl){
        $.ajax({
            ...
            data:'barcode='+$(".getBoothNumb").val(),  //请求参数
            success: function(dataList) {
                if(dataList=="0"){  //如果没有搜索到数据返回空数组
                    ...
                    var text = dataList[0]["company"];
                    $(".getCompany").val(text);
                    Pum.textAdaptation({LineNumber:1,event:{type:"input"},autoWrap:false,text:text,pElem:".card_company",dElem:".card_company div",iElem:".getCompany",minF:fontSize.companyMinFontSize});
                }
            },
