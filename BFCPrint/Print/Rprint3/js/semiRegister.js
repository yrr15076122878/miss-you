    ...
    selectData: function () {   //表格数据单行选中执行事件
        $(main.dataTableId).on('click', 'tbody tr', function () {
            ...
            if ($(this).hasClass('selected')) {
                ...
                var textC = $(this).find("td:eq(5)").text().toLocaleUpperCase();
    ...
    dataTables: function (resultData) {  //动态加载数据表
        ...
        var tableD = $(main.dataTableId).dataTable({
            ...
            "createdRow": function (row) {
                ...
                //if (pre === "Pre") {
                //    $('td', row).eq(6).html("<span pre='1' class='label'>预</span>");
                //} else {
                //    $('td', row).eq(6).html("<span pre='0' class='label'></span>");
                //}
